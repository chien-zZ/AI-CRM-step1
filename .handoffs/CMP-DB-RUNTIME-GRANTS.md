...
- `ai_crm_runtime` 在生产由已接受的 PostgreSQL 初始化脚本创建；模块隔离测试数据库可以不存在该生产角色。
...
- `pnpm --filter @ai-crm/database test`: 23 passed, 3 environment-gated PostgreSQL tests skipped.
- `pnpm db:test:integration`: 26 passed against isolated PostgreSQL 17.5, including all three database integration cases.
- `pnpm check`: repository checks 40/40 and Turbo tasks 140/140 passed.

## Review Pass

- Authorization: no business Permission/Role/Grant was added; database access follows current production composition and denies unlisted effective privileges, including those inherited from `PUBLIC`.
...
- Migrations: only new global migration `0013` was added; no historical migration or automatic synchronization was used. Metadata includes lock/data impact, recovery and forward-fix guidance.
