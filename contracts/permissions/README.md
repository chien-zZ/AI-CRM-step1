...
- `protected-policy-publication-command.v1.schema.json` defines the complete transport-neutral internal publication command: stable operation/publication IDs, authenticated actor reference, current Workforce Person/Assignment authorization context, reason, safe Trace reference, and a full immutable non-empty policy snapshot. It deliberately defines no production endpoint, publication permission, role, grant, Owner, or approval route.
