...
describe("API runtime configuration", () => {
  it("uses the reviewed container defaults", async () => {
    await expect(loadApiRuntimeConfiguration({ env: {} })).resolves.toEqual({
      environment: "development", host: "0.0.0.0", port: 3000, release: "development",
    });
  });

  it("rejects an invalid bind address and port", async () => {
    await expect(loadApiRuntimeConfiguration({ env: { AI_CRM_API_HOST: "public.example", AI_CRM_API_PORT: "0" } })).rejects.toThrow();
  });
  ...
  it("fails closed when production release or container binding is missing", async () => {
    ...
    await expect(loadApiRuntimeConfiguration({ env: { AI_CRM_API_HOST: "127.0.0.1", AI_CRM_RELEASE: "2026.07.27.1", NODE_ENV: "production" } })).rejects.toThrow("api_production_bind_invalid");
  });
});
