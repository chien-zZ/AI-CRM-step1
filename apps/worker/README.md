...
Production bootstrap deliberately stops after generic resource validation with the stable `task_projection_consumer_policy_unavailable` error. It closes the publisher connection, topology-free consumer-control connection, and database runtime within the configured drain budget, then exits non-zero. This is not a production consumer activation: exact Task projection attempts, delay layers, handler deadline, prefetch, concurrency, error classification, capacity, and alert policy remain unconfirmed. Startup performs no Outbox polling and never applies migrations.
