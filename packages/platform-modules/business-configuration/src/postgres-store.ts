...
import type { ConfigurationInvalidationEvent, DictionaryDraft, DictionaryItem, DictionaryRelease, ParameterActivation, ParameterDefinition, ParameterValueRelease } from "./types.js";
...
export function createPostgresBusinessConfigurationStore(runtime: ConfigurationPersistenceRuntime): BusinessConfigurationStore {
  ...
  return {
    ...
    listEffectiveActivations: async (parameterKey, scopes, at) => {
      ...
      const query = await runtime.execute<ActivationRow>("select * from business_configuration.parameter_activations where parameter_key=$1 and (scope_type,scope_reference) in (select * from unnest($2::text[],$3::text[])) and effective_from<=$4 and (effective_to is null or $4<effective_to)", [parameterKey, scopeTypes, scopeReferences, at]);
    ...
    activate: (input) => transaction(async () => {
      ...
      const overlap = await runtime.execute("select 1 from business_configuration.parameter_activations where parameter_key=$1 and scope_type=$2 and scope_reference=$3 and effective_from<coalesce($5::timestamptz,'infinity') and $4::timestamptz<coalesce(effective_to,'infinity') limit 1", [candidate.parameterKey, candidate.scope.scopeType, candidate.scope.scopeReference, candidate.effectiveFrom, candidate.effectiveTo ?? null]);
