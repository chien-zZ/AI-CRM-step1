...
class PostgresTaskCenterStore implements TaskCenterStore {
  ...
  public async list(input:{status?:TaskProjectionStatus;limit:number;cursor?:string}):Promise<TaskPage>{const result=await this.db.execute<ProjectionRow>(`select * from platform_task_center.task_projections where ($1::text is null or status=$1) and ($2::text is null or source_type || chr(0) || source_task_id > $2) order by source_type,source_task_id limit $3`,[input.status??null,input.cursor??null,input.limit+1]);const items=result.rows.slice(0,input.limit).map(map);const last=items.at(-1);return{items,...(result.rows.length>input.limit&&last?{nextCursor:`${last.sourceType}\u0000${last.sourceTaskId}`}:{})};}
