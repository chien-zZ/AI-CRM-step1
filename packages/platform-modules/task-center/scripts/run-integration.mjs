...
const secretDirectory=await mkdtemp(resolve(tmpdir(),"ai-crm-prc02-"));const project=`ai-crm-test-prc02-${randomUUID().slice(0,8)}`;const pnpmCli=process.env.npm_execpath;if(!pnpmCli)throw new Error("pnpm CLI path is unavailable.");
const environment={...process.env,AI_CRM_COMPOSE_SECRET_DIR:secretDirectory,AI_CRM_CREATE_TEST_MIGRATION_URL:"1",AI_CRM_TEST_POSTGRES_PORT:String(port),DATABASE_MIGRATION_URL_FILE:resolve(secretDirectory,"migration_url"),TEST_TASK_CENTER_DATABASE_URL_FILE:resolve(secretDirectory,"migration_url")};
const compose=["compose","-p",project,"-f","deploy/compose/compose.base.yml","-f","deploy/compose/compose.postgres-test.yml"];
const run=(command,args)=>{const result=spawnSync(command,args,{cwd:resolve(import.meta.dirname,"../../../.."),env:environment,shell:false,stdio:"inherit"});if(result.status!==0)throw new Error(`${command} ${args[0]??""} failed.`);};
