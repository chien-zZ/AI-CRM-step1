...
export class MemoryFileCenterStore implements FileCenterStore {
  ...
  async completeUpload(input: Parameters<FileCenterStore["completeUpload"]>[0]) {
    ...
    if (joined.session.status !== "created" || joined.contentVersion.status !== "awaiting_upload") throw new FileCenterError("file_center_operation_conflict");
