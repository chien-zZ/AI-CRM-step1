...
import { MemoryFileCenterStore, createFileCenterService, type FileAudit, type FileAuthorizer, type MalwareScanner, type StorageAdapter, type StorageObjectMetadata } from "./index.js";
...
class FakeStorage implements StorageAdapter {
  ...
  readObject = vi.fn(({ objectHandle }: { readonly objectHandle: string }) => { const bytes = this.objects.get(objectHandle)?.bytes; return bytes ? Promise.resolve(bytes) : Promise.reject(new Error("missing")); });
...
describe("File Center service", () => {
  ...
  it("reconciles a missing object to an explicit unavailable state", async () => {
    const runtime = setup(); const created = await create(runtime.service); const command = { ...meta(), contentVersionId: created.fileReference.contentVersionId }; const result = await runtime.service.reconcileContentVersion(command); expect(result.contentVersion.status).toBe("object_missing"); const handle = runtime.storage.uploadHandles.at(-1); if (!handle) throw new Error("missing handle"); runtime.storage.objects.set(handle, { bytes: new TextEncoder().encode("fixture"), mediaType: "text/plain" }); const replay = await runtime.service.reconcileContentVersion(command); expect(replay).toEqual({ contentVersion: result.contentVersion, replayed: true }); expect(runtime.storage.inspectObject).toHaveBeenCalledOnce();
  });
