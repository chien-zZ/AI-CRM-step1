  ...
  return Object.freeze({ async publishBatch() {
    ...
    for (const row of records) {
      try {
        ...
        await store.markOutboxFailure({ messageId: row.messageId, claimToken: row.claimToken, at: clock(), availableAt: new Date(clock().getTime() + seconds * 1000), errorCode: error instanceof EventingError ? error.code : "publish_failed", isolate: exhaust }).catch(() => false);
