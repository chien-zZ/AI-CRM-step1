...
export const validateVariables=(policy:WorkflowVariablePolicy,definitionKey:string,variables:Readonly<Record<string,WorkflowVariable>>):Readonly<Record<string,WorkflowVariable>>=>{const rules=policy.definitions[definitionKey];if(rules===undefined||Object.keys(variables).length>32)throw new WorkflowError("WORKFLOW_INVALID_INPUT");for(const[name,value]of Object.entries(variables)){const kind=rules[name];if(kind===undefined)throw new WorkflowError("WORKFLOW_INVALID_INPUT");validateVariable(value,kind);}return Object.freeze({...variables});};
